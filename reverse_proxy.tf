resource "azurerm_public_ip" "public-ip-appgateway" {
  name                = "ip-appgateway-terraform-dev-francecentral"
  resource_group_name = azurerm_resource_group.rg.name
  location            = "France Central"
  allocation_method   = "Static"
  sku = "Standard"
}

locals {
  backend_address_pool_name      = "${azurerm_virtual_network.vn.name}-beap"
  frontend_port_name             = "${azurerm_virtual_network.vn.name}-feport"
  frontend_ip_configuration_name = "${azurerm_virtual_network.vn.name}-feip"
  http_setting_name              = "${azurerm_virtual_network.vn.name}-be-htst"
  listener_name                  = "${azurerm_virtual_network.vn.name}-httplstn"
  request_routing_rule_name      = "${azurerm_virtual_network.vn.name}-rqrt"

}

resource "azurerm_application_gateway" "appgateway" {
  name                = "appgateway-terraform-dev-francecentral"
  resource_group_name = azurerm_resource_group.rg.name
  location            = "France Central"

  sku {
    name     = "Standard_v2"
    tier     = "Standard_v2"
    capacity = 2
  }

  gateway_ip_configuration {
    name      = "gateway-ip-configuration"
    subnet_id = azurerm_subnet.appgateway.id
  }

  frontend_port {
    name = local.frontend_port_name
    port = 80
  }

  frontend_ip_configuration {
    name = local.frontend_ip_configuration_name
    public_ip_address_id = azurerm_public_ip.public-ip-appgateway.id
  }

  backend_address_pool {
    name = local.backend_address_pool_name
    ip_addresses = [ azurerm_container_group.aci-terraform-dvwa.ip_address ]
  }

  backend_http_settings {
    name = local.http_setting_name
    cookie_based_affinity = "Disabled"
    port = 80
    protocol = "Http"
    request_timeout = 60
  }

  http_listener {
    name                           = local.listener_name
    frontend_ip_configuration_name = local.frontend_ip_configuration_name
    frontend_port_name             = local.frontend_port_name
    protocol                       = "Http"
  }

  request_routing_rule {
    name                       = local.request_routing_rule_name
    priority                   = 9
    rule_type                  = "Basic"
    http_listener_name         = local.listener_name
    backend_address_pool_name  = local.backend_address_pool_name
    backend_http_settings_name = local.http_setting_name
  }
}